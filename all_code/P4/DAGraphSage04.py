...
if __name__ == "__main__":
    ...
    if  load_matrix:
        ...
        #A = np.load('/home/user/anaconda3/envs/zhaotengdemo/liucheng/集成的代码/P4/data/PEMS04/all_A(train,val,test).npy')
        # A[np.isnan(A)] = 0.
        # A[np.isinf(A)] = 0.
        # A = A.mean(0) * KMD
        # if mat_A_and_Diag:
    ...
    # #np.save('/home/user/liucheng/data/predict_event(batch=2).npy', fake_event)
    # #torch.save(G, '/home/user/liucheng/data/gan(04所有邻接矩阵,lr=0.1,event_flag = 1e-2,batch=2).pkl')
    # #fake_event = np.load('/home/user/anaconda3/envs/zhaotengdemo/liucheng/集成的代码/P4/data/PEMS04/predict_event_all(404).npy')
    ...
    # print(predict_loss.item())
    A_fake = np.load('/home/user/liucheng/data/0预测的04邻接矩阵(batch_size=2,最终版).npy')
    #A_fake = A_fake.detach().cpu().numpy()
    A_fake[A_fake<=1e-9] = 0
    print('\nPredict A complete.')
    #A[A < 1e-9] = 0
    A = A_fake
    A_train = A[:242]
    ...
    for epoch in range(1, epochs + 1):
        ...
        _MAE, _RMSE, _MAPE,_,_,_ = evaluate(net, test_loader, true_value, A_test, device, epoch_=epoch)
        ...
        print(f"\t >>> Lowest v-loss in {epoch} :  epoch_{arg + 1}  {validation_loss_lst[arg]}  lr = {scheduler.get_last_lr()}\n\n")
        if validation_loss_lst[arg] < 925 and learning_rate == 0.001:
