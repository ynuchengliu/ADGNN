...
def predict(net, test_loader, supports, device):
	...
	with torch.no_grad():
		...
		# 	# test_w = test_w.to(device)
		# 	# test_d = test_d.to(device)
		# 	#if index>=3 and temp%5==0 and index<154:
		# 	if  index < 154:
		# 		test_r = test_r.to(device)
		# 		test_t = test_t.to(device)
		# 		_, spatial_at, temporal_at = net(test_r, supports[temp2])
...
def evaluate(net, test_loader, true_value, supports, device, epoch_):
	...
	with torch.no_grad():
		...
		ture = ture.reshape(-1,307,12)
		true_value = np.concatenate([ture,last_ture],0)
		#true_value = true_value[:1280]
	...
	return _MAE, _RMSE, _MAPE,mae,rmse,mape
